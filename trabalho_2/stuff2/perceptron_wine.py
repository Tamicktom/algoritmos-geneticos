"""
Implementação do Perceptron para classificação do dataset Wine
Exercício 2 - Computação Inspirada pela Natureza
"""
...
def main():
    ...
    print("Carregando o dataset Wine...")
    
    # Ler o arquivo wine.data
    # O arquivo não tem cabeçalho, então especificamos os nomes das colunas
    # A primeira coluna é a classe (1, 2 ou 3)
    column_names = ['class', 'Alcohol', 'Malic_acid', 'Ash', 'Alcalinity_of_ash', 
                   'Magnesium', 'Total_phenols', 'Flavanoids', 'Nonflavanoid_phenols', 
                   'Proanthocyanins', 'Color_intensity', 'Hue', 'OD280/OD315', 'Proline']
    
    data = pd.read_csv('/home/ubuntu/exercicio2/wine.data', header=None, names=column_names)
    
    # Extrair atributos e classes
    X = data.iloc[:, 1:].values  # Todas as colunas exceto a primeira (classe)
    y_labels = data.iloc[:, 0].values  # Primeira coluna (classe)
    
    # Ajustar as classes para começar de 0 (originalmente são 1, 2, 3)
    y = y_labels - 1
    
    # Nomes das classes
    target_names = [f"Classe {i+1}" for i in range(len(np.unique(y)))]
